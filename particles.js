...
function handleParticles(){
    ...
    if (((keys[37] || keys[38] || keys[39] || keys[40])) && grandpa.y > 250 && particlesArray.length < maxParticles + 10){
        for (let i = 0; i < 10; i++){
            particlesArray.unshift(new Particle(grandpa.x, grandpa.y));
        }
    }
}
...
function handleRipples(){
    ...
    if (((keys[37] || keys[38] || keys[39] || keys[40])) && grandpa.y < 250 && grandpa.y > 100){
        for (let i = 0; i < 20; i++){
