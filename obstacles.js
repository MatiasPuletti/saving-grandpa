...
function initObstacles() {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    let x = i * 350;
    virusArray.push(
      new Obstacle(x, canvas.height - grid * 2 - 20, grid, grid, 1, 'virus')
    );
  }
  // Lane 2 - More virus
  for (let i = 0; i < 2; i++) {
    let x = i * 300;
    virusArray.push(
      new Obstacle(x, canvas.height - grid * 3 - 20, grid, grid, -5, 'virus')
    );
  }
  ...
  for (let i = 0; i < 2; i++) {
    let x = i * 400;
    virusArray.push(
      new Obstacle(x, canvas.height - grid * 4 - 20, grid, grid, 2, 'virus')
    );
  }
  ...
  for (let i = 0; i < 2; i++) {
    let x = i * 400;
    logsArray.push(
      new Obstacle(x, canvas.height - grid * 5 - 20, grid, grid, -2, 'log')
    );
  }
  // Lane 5 - Turtles
  for (let i = 0; i < 3; i++) {
