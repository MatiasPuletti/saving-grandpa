...
function initObstacles(){
  ...
  for (let i = 0; i < 2; i++){
      let x = i * 350;
      virusArray.push(new Obstacle(x, canvas.height - grid * 2 - 20, grid * 2, grid, 1, 'car'));
  }
  // lane 2
  for (let i = 0; i < 2; i++){
      let x = i * 300;
      virusArray.push(new Obstacle(x, canvas.height - grid * 3 - 20, grid * 2, grid, -2, 'car'))
  }
  // lane 3 
  for (let i = 0; i < 2; i++){
      let x = i * 400;
      virusArray.push(new Obstacle(x, canvas.height - grid * 4 - 20, grid * 2, grid, 2, 'car'));
  }
  // lane 4
  for (let i = 0; i < 2; i++){
      let x = i * 400;
...
function handleObstacles(){
  ...
  for (let i = 0; i < virusArray.length; i++){
      if (collision(grandpa, virusArray[i])){
          ctx4.drawImage(collisions, 0, 100, 100, 100, grandpa.x, grandpa.y, 50, 50);
          resetGame();
      }
  }
